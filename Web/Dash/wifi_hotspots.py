...
map_data = pd.read_csv("Data/nyc-wi-fi-hotspot-locations.csv")
            ...
            [
                ...
                html.Div(
                    [
                        ...
                        dcc.Dropdown(
                            id='type',
                            options=[{'label': str(item), 'value': str(item)} for item in set(map_data['Type'])],
                            multi=True,
...
def update_selected_row_indices(type, boroughs):
    map_aux = map_data.copy()

    # Type filter
    map_aux = map_aux[map_aux['Type'].isin(type)]
    # Boroughs filter
    map_aux = map_aux[map_aux["Borough"].isin(boroughs)]
